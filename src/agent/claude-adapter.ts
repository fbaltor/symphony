...
export interface ClaudeQueryMessage {
  ...
  usage?: {
    ...
    /**
     * B-3 / D-21 (AGENT-529): prompt-caching token tracking.
     *
...
export type ClaudeQueryFn = (args: {
  ...
  options: {
     ...
     * declines to push commits — observed on AGENT-447 across 11
    ...
    systemPrompt?:
      ...
      | {
          ...
          append?: string;
          /**
           * B-3 / D-21 (AGENT-529): when true, the SDK omits the per-session
...
export type ClaudeMcpServerOption =
  ...
  | {
      // A-12 / S-D10: in-process MCP server. The Claude Agent SDK accepts
...
export interface ClaudeAdapterOptions {
  model?: string;
  /**
   * A-10: forwarded to Anthropic's adaptive thinking config as
...
export class ClaudeAgentRunner implements AgentRunner {
  ...
  async startSession(args: StartSessionArgs): Promise<AgentSessionHandle> {
    ...
    // A-22: real event queue replaces the previous placeholder. The
  ...
  async runTurn(session: AgentSessionHandle, args: RunTurnArgs): Promise<RunTurnResult> {
    ...
    let inputTokens = 0;
    let outputTokens = 0;
    // B-3 / D-21: prompt-cache observability. `cache_creation_input_tokens`
    ...
    // A-29: per-issue cap tracking. When `perIssueCapUsd` is set,
    ...
    // B-6: ALSO check the per-state turn cap (covers the case where one
    ...
    // A-22: pull the per-session event queue off the handle ONCE at the
    ...
    try {
      const stream = queryFn({
        ...
        options: {
          ...
          //   - A-24 (bash deny patterns: sudo, cd /, curl, wget, …)
          //   - Symphony self-edit lock (blocks writes to own source tree)
          //   - B-12 (per-state writeCwds restriction)
          //   - workspace-bound Edit/Write path containment (spec §9.5)
          // — is silently defeated when `bypassPermissions` is set.
          //
          // Surfaced by AGENT-489 (T-004 Implement turn ran 20 internal
          // turns generating 35K tokens; agent reported `canUseTool was
          // never invoked on this dispatch path`) and AGENT-496 (T-007
          // confirmed `Read /etc/passwd` succeeded without any guard
          // firing). `gcloud logging read … "canUseTool"` returned ZERO
          // results across the dispatch windows of both tickets.
          //
          ...
          abortController: ac,
          // A-24 / S-D19: defense-in-depth. canUseTool is invoked by the
          ...
          // refuses to write code. Observed on AGENT-447 Implement-stage:
          ...
          // Observed on AGENT-447: 11 identical "Implement turn N still
          ...
          systemPrompt: {
            type: "preset",
            preset: "claude_code",
            // B-3 / D-21 (AGENT-529): cache the static preset prefix
          ...
          // A-10: forward agent_runtime.effort to Anthropic's adaptive
      ...
      // A-22: classify per-message SDK events into the spec-defined
      ...
      for await (const msg of stream) {
        ...
        // A-22: classify SDK messages into the spec-defined AgentEventKind
        ...
        if (msg.usage) {
          ...
          // B-3 / D-21: track cache-fee subsets so the audit row + Slack
        ...
        if (typeof msg.total_cost_usd === "number") {
          ...
          // A-29: mid-stream cap check. The SDK surfaces total_cost_usd on
          ...
          if (cap > 0 && alreadyRecorded + costUsd > cap) {
            ...
            // B-6: per-state cap independently. THIS turn alone exceeded
      ...
      if (capExceeded) {
        // A-29 / B-6: cap-driven abort takes precedence over the generic
    ...
    // A-22: also push the final outcome event into the live queue so
    ...
    return {
      ...
      usageDelta: {
        ...
        totalTokens: inputTokens + outputTokens,
        costUsd,
        // B-3 / D-21: forward cache-fee subsets to the orchestrator so
  ...
  async endSession(session: AgentSessionHandle): Promise<void> {
    ...
    // A-22: close the per-session event queue so any consumer iterating
...
/**
 * A-25 — `attachWorkspace` and `extractWorkspaceFromSession` (Symbol-keyed
